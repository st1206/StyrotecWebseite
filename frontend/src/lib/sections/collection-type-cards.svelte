<script lang="ts">
	...
	const brochures = $derived.by(() => {
		...
		return { brochures: validBrochures };
	});
	...
	const fairs = $derived.by(() => {
		const validFairs = dataEntries
			.map((entry: any) => {
				...
				return {
					...
					content: entrySafe.getString('content'),
		...
		return { fairs: validFairs };
	});

	const hasValidData = $derived.by(() => {
		switch (dataType) {
			case 'brochures':
				return brochures.brochures.length > 0;
			case 'fairs':
				return fairs.fairs.length > 0;
			default:
...
{:else if !hasValidData}
	...
	<div class="py-16 text-center">
		...
		<h3 class="text-muted-foreground mb-2 text-lg font-semibold">No {dataType} available</h3>
...
{:else if dataType === 'brochures'}
	<Brochures brochures={brochures.brochures as any} />
{:else if dataType === 'fairs'}
	<Fairs fairs={fairs.fairs as any} />
{:else if dataType === 'testimonials'}
	<!-- Testimonials not implemented yet -->
	<div class="py-16 text-center">
		<div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-lg bg-yellow-100">
		...
		<h3 class="mb-2 text-lg font-semibold text-yellow-800">Testimonials not implemented</h3>
		<p class="text-sm text-yellow-700">The testimonials collection type is not yet implemented</p>
	</div>
{:else}
