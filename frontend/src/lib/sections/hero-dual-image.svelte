...
<section class="mt-20 lg:container lg:mx-auto lg:mt-32 lg:w-full">
		...
		<div class="relative">
			...
			<img
				src={!PUBLIC_BACKEND_URL.includes('https')
					...
					: data.image.url}
				alt="FS10 Titelbild"
				class="shadow-primary bg-secondary z-20 h-[300px] w-full object-cover lg:h-[600px]"
			/>
			<BlurFade once={true} delay={0.2} duration={0.3}>
				<div
					class="bg-primary absolute bottom-0 right-0 hidden h-[200px] w-2/5 [clip-path:polygon(50%_50%,100%_0%,100%_100%,0%_100%)] lg:block"
				></div>
			</BlurFade>
		</div>
	</BlurFade>
	...
	<div class="relative w-full">
		...
		<BlurFade once={true} delay={0.1} duration={0.3}>
			<div
				class="shadow-primary absolute right-0 hidden h-[600px] w-2/5 -translate-y-[194px] lg:block"
			>
				<div
				...
				<img
					src={!PUBLIC_BACKEND_URL.includes('https')
						? `${PUBLIC_BACKEND_URL}${data.basicTextImage.image.formats['large']?.url || data.basicTextImage.image.url}`
						: data.basicTextImage.image.url}
					alt={data.basicTextImage.image.alternativeText}
