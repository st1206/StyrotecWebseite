...
export const load: LayoutLoad = async ({ url, data }) => {
	...
	if (!/^(de|en)(\/|$)/.test(segments[0] ?? '')) {
		...
		return goto(`/${prefix}${pathname}`, { replaceState: true });
	}
	...
	if (!incomingSlug) {
		return goto(`/${lang}/${DEFAULT_SLUG[lang]}`, { replaceState: true });
	}
	...
	if (pathname === '/de/produkte') {
		return goto('/de/produkte/portalfraesmaschinen', { replaceState: true });
	}
	if (pathname === '/en/products') {
		return goto('/en/products/gantry-machines', { replaceState: true });
	}
